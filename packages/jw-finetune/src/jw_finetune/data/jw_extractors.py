    ...
    *,
    ...
    for note in notes:
        ...
        if language.startswith("es"):
            question = f"Explica {ref}: ¿qué significa «{headword}»?" if headword else f"Explica {ref}."
        else:
            question = f"Explain {ref}: what does “{headword}” mean?" if headword else f"Explain {ref}."
        yield QAPair(
            question=question,
            answer=body,
        ...
        for ref in refs[:max_refs]:
            ...
            try:
                if key not in seen_chapters:
                    _url, html = await wol_client.get_bible_chapter(ref.book_num, ref.chapter, language=language)
                ...
                verse_text = _extract_single_verse(seen_chapters[key], ref.verse_start)
                if verse_text:
                    ref_key = _format_verse_ref(ref.book_num, ref.chapter, ref.verse_start, language)
...
def extract_user_notes_qa(
    ...
    *,
    ...
    for note in backup.notes:
        ...
        if language.startswith("es"):
            question = title or (f"¿Qué notas tomé sobre {loc_str}?" if loc_str else "Nota personal.")
        else:
            question = title or (f"What notes did I take on {loc_str}?" if loc_str else "Personal note.")
        yield QAPair(
            question=question,
            answer=body,
        ...
        for section in getattr(week, "sections", []) or []:
            ...
            for assn in section.assignments or []:
                ...
                if language.startswith("es"):
                    ...
                    q = f"In the «{section_label}» section of the meeting, how would you prepare «{title}» {minutes}?"
