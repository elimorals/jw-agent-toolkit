...
async def test_async_synth_basic(tmp_path: Path) -> None:
    ...
    valid_response = json.dumps(
        {
            "pairs": [
                {"q": "¿Cuál es el propósito?", "a": "El propósito está claramente expuesto en las Escrituras."},
            ]
        }
    )
...
async def test_async_synth_uses_cache_on_rerun(tmp_path: Path) -> None:
    ...
    valid_response = json.dumps(
        {
            "pairs": [
                {"q": "¿Qué representa?", "a": "Representa una verdad bíblica fundamental para los TJ."},
            ]
        }
    )
...
async def test_async_synth_progress_callback(tmp_path: Path) -> None:
    ...
    valid_response = json.dumps(
        {
            "pairs": [
                {"q": "¿Qué es?", "a": "Es un concepto fundamental en la doctrina bíblica enseñada por los Testigos."},
            ]
        }
    )
...
async def test_async_synth_handles_provider_failures(tmp_path: Path, monkeypatch) -> None:
    ...
    class FlakyProvider:
        ...
        def generate(self, req):
            ...
            return LLMResponse(
                text=json.dumps(
                    {"pairs": [{"q": "¿Por qué?", "a": "Porque la Biblia lo enseña claramente en numerosos pasajes."}]}
                ),
                provider="flaky",
                model="f-1",
